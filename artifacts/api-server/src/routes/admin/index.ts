...
router.patch("/bookings/:id", async (req, res) => {
  try {
    ...
    if (driverId !== undefined) {
      ...
      if (status === undefined) {
        if (driverId !== null && (current.status === "pending" || current.status === "confirmed")) {
          updates.status = "assigned";
        } else if (driverId === null && current.status === "assigned") {
...
router.get("/vehicles", async (_req, res) => {
  try {
    ...
    res.status(500).json({ error: err?.message ?? "Failed to load vehicles" });
  }
});
...
router.get("/zones", async (_req, res) => {
  try {
    ...
    res.status(500).json({ error: err?.message ?? "Failed to load zones" });
  }
});
