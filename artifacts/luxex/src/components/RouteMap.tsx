    ...
    return new Promise((resolve) => {
      ...
      service.route(
        ...
        (result: any, status: string) => {
          ...
          resolve({ distanceMiles: (distanceMeters / 1609.344) * 1.05, durationMin: Math.round(durationSeconds / 60), polyline: [] });
        }
      );
    });
  } catch {
...
export function RouteMap({ pickup, dropoff, stops = [], className = "" }: RouteMapProps) {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      directionsRenderer.current = new goog.maps.DirectionsRenderer({
        ...
        polylineOptions: { strokeColor: "#F2E147", strokeWeight: 4, strokeOpacity: 0.9 },
      });
  ...
  useEffect(() => {
    ...
    if (pickup) {
      markersRef.current.push(new goog.maps.Marker({
        position: { lat: pickup.lat, lng: pickup.lon }, map: mapInstance.current,
        icon: { path: goog.maps.SymbolPath.CIRCLE, scale: 9, fillColor: "#F2E147", fillOpacity: 1, strokeColor: "#000", strokeWeight: 3 },
        title: "Pickup", zIndex: 10,
      }));
    }

    stops.forEach((s, i) => {
      markersRef.current.push(new goog.maps.Marker({
        position: { lat: s.lat, lng: s.lon }, map: mapInstance.current,
        icon: { path: goog.maps.SymbolPath.CIRCLE, scale: 8, fillColor: "#a78bfa", fillOpacity: 1, strokeColor: "#000", strokeWeight: 2 },
        label: { text: String(i + 1), color: "#000", fontSize: "8px", fontWeight: "900" },
        title: `Stop ${i + 1}`, zIndex: 9,
      }));
    });

    if (dropoff) {
      markersRef.current.push(new goog.maps.Marker({
        position: { lat: dropoff.lat, lng: dropoff.lon }, map: mapInstance.current,
        icon: { path: goog.maps.SymbolPath.CIRCLE, scale: 9, fillColor: "#ffffff", fillOpacity: 1, strokeColor: "#000", strokeWeight: 3 },
        title: "Drop-off", zIndex: 10,
      }));
    }
    ...
    if (allPoints.length >= 2) {
      ...
      const waypoints = allPoints.slice(1, -1).map((p: LatLng) => ({ location: { lat: p.lat, lng: p.lon }, stopover: true }));
      ...
      service.route(
        ...
        (result: any, status: string) => {
          ...
          if (status === "OK" && directionsRenderer.current && result?.routes?.[0]) {
            ...
            setRouteInfo({ distanceMiles: (distanceMeters / 1609.344) * 1.05, durationMin: Math.round(durationSeconds / 60), polyline: [] });
          } else {
      ...
      if (directionsRenderer.current) directionsRenderer.current.setDirections({ routes: [] });
      if (allPoints.length === 1) { mapInstance.current.setCenter({ lat: allPoints[0].lat, lng: allPoints[0].lon }); mapInstance.current.setZoom(14); }
    }
